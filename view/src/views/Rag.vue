    ...
    <div class="page-wrapper">
      <div class="page-card">
        <div class="page-header">
          ...
          <div>
            <h1>文档导入 · 向量数据库</h1>
            <div class="subtitle">将 TXT 文档向量化存入 RAG 知识库（最多 {{ MAX_FILES }} 个文件）</div>
          </div>
        </div>
        ...
        <el-form ref="formRef" :model="form" :rules="rules" label-width="100px" label-position="right" size="default">
          ...
          <el-form-item label="切块策略" prop="chunkStrategy">
            <el-select v-model="form.chunkStrategy" placeholder="请选择切块策略" style="width: 100%" clearable>
              <el-option v-for="item in chunkStrategies" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="选择文档" prop="files">
            <div class="upload-area" @click="triggerUpload" v-if="form.files.length === 0">
              ...
              <div class="upload-hint">仅支持 .txt 文本文件，最多 {{ MAX_FILES }} 个</div>
            </div>
              ...
              <div class="file-list">
                <div class="file-item" v-for="(file, index) in form.files" :key="index">
                  <div class="file-left">
                    ...
                    <div class="file-error" v-if="fileErrors[file.name]">
                      <el-icon><WarningFilled /></el-icon>
                      <span>{{ fileErrors[file.name] }}</span>
                    </div>
                  </div>
                  <el-button class="file-remove" type="danger" size="small" :icon="Delete" plain @click="removeFile(index)">删除</el-button>
                </div>
              </div>
              <div class="file-count-hint" v-if="form.files.length > 0">
                已选择 {{ form.files.length }} / {{ MAX_FILES }} 个文件
              </div>
              <el-button
                v-if="form.files.length < MAX_FILES"
                type="primary" plain size="small" :icon="Plus"
                @click="triggerUpload" style="margin-top: 8px">
                继续添加文档
              </el-button>
              <el-tag v-else type="warning" size="small" style="margin-top: 8px">已达最大文件数</el-tag>
            </div>
...
<script setup>
import { Upload, FolderOpened, Document, Delete, Plus, WarningFilled } from '@element-plus/icons-vue'
import { useRag } from '../composables/useRag'

const {
  MAX_FILES,
  docTypes, departments, chunkStrategies,
  form, rules,
  uploading, progressPercent, progressStatus, progressText, importResult,
  fileErrors, formRef, fileInput,
